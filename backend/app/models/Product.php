...
class Product extends DB
{
    ...
    {
        $sql = "SELECT p.id_produit,p.nom,p.description,p.prix,p.image,c.nom_cat,p.quantite FROM produit p,categories c  where p.id_category=c.id_cat and p.quantite!=0 ORDER BY p.id_produit Desc";
        $sql = $this->connect()->prepare($sql);
        if ($sql->execute())
    ...
    public function search_by_category($id_cat){
        $sql="SELECT * from produit where id_category=? and quantite!=0";
        $sql=$this->connect()->prepare($sql);
